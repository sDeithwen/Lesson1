...
function showFullInfo(rowId) {
    let row = document.getElementById(rowId);
    let label = createFullInfoLabel();
    label.innerHTML = `<strong>Название товара: </strong> ${row.children[0].innerText} </br>` +
        `<strong>Цена товара: </strong> ${row.children[1].innerText} </br>` +
        `<strong>Количество на складе: </strong> ${row.children[2].innerText} </br>`;
}
...
function createTable() {
    ...
    addEventListenerToTable(tbl);
}

function addEventListenerToTable(table) {
